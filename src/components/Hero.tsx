    ...
    <section id="about" className="relative min-h-screen flex items-center justify-center pt-16 overflow-hidden">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        <h1 className="text-5xl md:text-7xl font-serif font-bold text-slate-100 mb-6 tracking-tight">
          Hello, I'm <span className="text-transparent bg-clip-text bg-gradient-to-r from-serene-300 to-accent-gold">{PROFILE.name}</span>
        </h1>
        ...
        <div className="glass-panel p-8 rounded-2xl max-w-3xl mx-auto mb-10 shadow-lg shadow-serene-900/50">
          {PROFILE.about.map((paragraph, index) => (
            <p key={index} className={`text-slate-300 text-lg leading-relaxed ${index > 0 ? 'mt-4' : ''}`}>
              {paragraph}
            </p>
          ))}
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
          <a href="#projects" className="px-8 py-3 rounded-full bg-serene-600 hover:bg-serene-500 text-white font-medium transition-all shadow-lg shadow-serene-600/20">
            View Projects
          </a>
          <a href="#contact" className="px-8 py-3 rounded-full bg-transparent border border-slate-600 hover:border-serene-300 text-slate-300 hover:text-white transition-all">
            Contact Me
          </a>
        </div>
      </div>
