...
const Projects: React.FC = () => {
  return (
    <section id="projects" className="py-15 relative">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-12">
          ...
          <p className="text-slate-400 max-w-2xl">
            A selection of my work in Cybersecurity, AI, and Full Stack
            Development.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-20">
          {PROJECTS.map((project, index) => (
            <div
              key={index}
              className="glass-panel rounded-xl p-6 flex flex-col hover:transform hover:-translate-y-1 transition-all duration-300 hover:shadow-lg hover:shadow-serene-500/10 group"
            >
              <div className="flex justify-between items-start mb-4">
                ...
                {project.link && (
                  <a
                    ...
                    className="text-slate-600 group-hover:text-serene-400 transition-colors"
              ...
              <div className="flex flex-wrap gap-2 mt-auto">
                {project.tech?.map((t, i) => (
                  <span
                    key={i}
                    className="px-2 py-1 text-xs rounded-md bg-serene-800 text-serene-300 border border-serene-700"
                  >
                    {t}
                  </span>
                ))}
        ...
        <div className="mt-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {CERTIFICATIONS.map((cert, index) => (
              <div
                key={index}
                className="glass-panel p-6 rounded-xl border-l-4 border-l-accent-gold"
              >
                <h3 className="text-lg font-bold text-slate-100 mb-2">
                  {cert.name}
                </h3>
                <p className="text-sm text-accent-gold font-mono mb-3">
                  {cert.date}
                </p>
